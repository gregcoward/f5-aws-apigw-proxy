...
when HTTP_REQUEST_DATA {
    ...
    #log local0.info "Here is content: $payload"
    ...
    if { [class match [string tolower $orig_path] contains aws-apis]}{
        ...
        if {[catch {ILX::call $RPC_HANDLE "apigw_proxy_call" $apiUrl $payload} result]} {
        ...
        if { [ lindex $result 0] eq "success"} {
            HTTP::respond 200 content "Successful call"
        } else {
            HTTP::respond 200 content "Call initiated but failed"
        }
